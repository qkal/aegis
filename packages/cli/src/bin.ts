#!/usr/bin/env node
/**
 * Executable entry point for the `aegisctx` binary.
 *
 ...
 * effect runs exactly when the `aegisctx` bin is invoked, and never
 * when an external consumer imports `aegisctx` for its typed API.
 */
...
run().then(
	...
	(err: unknown) => {
		process.stderr.write(`aegisctx: fatal: ${(err as Error).stack ?? String(err)}\n`);
		process.exit(1);
	},
);
